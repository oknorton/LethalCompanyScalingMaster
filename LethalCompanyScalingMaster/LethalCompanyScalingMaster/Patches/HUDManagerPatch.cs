...
namespace LethalCompanyModV2.Patches
{
    ...
    class DeathPenaltyPatch
    {
        static bool  Prefix(ref int playersDead, ref int bodiesInsured)
        {
            ...
            HUDManager.Instance.statsUIElements.penaltyAddition.text = string.Format("{0} casualties: -{1}%\n({2} bodies recovered)", (object)playersDead, (object)(float)((double)num * 100.0 * (double)(playersDead - bodiesInsured)), (object)bodiesInsured);
            HUDManager.Instance.statsUIElements.penaltyTotal.text = string.Format("DUE: ${0}", (object)(groupCredits - objectOfType.groupCredits));
